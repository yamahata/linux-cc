...
#define TDX_GET_QUOTE 0x10002
#define TDX_REPORT_FATAL_ERROR 0x10003
#define TDX_SETUP_EVENT_NOTIFY_INTERRUPT 0x10004
...
#define TDX_INSTRUCTION_PCONFIG 65

#define TDX_TEST_PORT 0x33
#define TDX_SUCCESS_PORT 0x33
#define TDX_DATA_REPORT_PORT 42
#define TDX_IO_READ 0
#define TDX_IO_WRITE 1
#define TDX_MMIO_READ 0
#define TDX_MMIO_WRITE 1

#define TDX_TDCALL_INSTRUCTION_LEAF_INFO   1
 ...
 */
static inline uint64_t tdvmcall_io(uint64_t port, uint64_t size, uint64_t write,
				   uint64_t *data)
{
 ...
 */
static inline void tdvmcall_fatal_with_data(uint64_t error_code, uint64_t data_gpa)
{
	struct kvm_regs regs;
...
{
	tdvmcall_fatal_with_data(error_code, /*data_gpa=*/0);
}
				      ...
				      uint64_t *r10, uint64_t *r11)
{
	...
	memset(&regs, 0, sizeof(regs));
	regs.rax = TDX_TDCALL_INSTRUCTION_LEAF_INFO;
	tdcall(&regs);
